...
print("\n" + "="*55)
print("  LOG-LINEAR REGRESSION - Nifty50 (INR)")
print("="*55)
...
ax = axes[0]
ax.scatter(close.index, log_price, s=0.8, color='steelblue', alpha=0.6, label='log(Nifty50 INR)')
...
ax = axes[1]
ax.plot(close.index, close.values, color='steelblue', linewidth=1, label='Actual Price (INR)')
...
ax = axes[2]
ax.bar(close.index, residuals, width=2, color='steelblue', alpha=0.6)
ax.axhline(0, color='black', linewidth=0.8, linestyle='--')
ax.set_title("Residuals (log space)", fontweight='bold')
ax.set_xlabel("Date")
ax.set_ylabel("Residual")

plt.suptitle(f"Nifty50 (INR) - Log-Linear Regression  |  "
             f"CAGR = {CAGR*100:.2f}%  |  R2 = {R2:.4f}  |  "
             f"Std Dev = {std_dev:.4f}",
             fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig('nifty50_inr_regression.png', dpi=150, bbox_inches='tight')
print("\nPlot saved: nifty50_inr_regression.png")
