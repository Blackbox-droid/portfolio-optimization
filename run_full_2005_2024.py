...
print(f"  Missing    : {nifty_csv.isna().sum()}")
...
nifty_yf  = download('^NSEI',    YF_START, END_DATE)
usdinr_yf = download('USDINR=X', YF_START, END_DATE)
nifty_yf_usd = (nifty_yf / usdinr_yf).rename('Nifty50_USD')
valid = nifty_yf_usd.dropna()
print(f"  Nifty50_USD (YF)  rows={len(valid)}  "
      f"{valid.index[0].date()} -> {valid.index[-1].date()}")

# STEP 3: Stitch Nifty50_USD - CSV (2005-2019) + YF (2020-2024)
...
print(f"  Missing    : {nifty_full.isna().sum()}")

# STEP 4: Download SP500, Gold, IEF from YF (2005-2024)
YF_TICKERS = {
    'SP500'  : '^GSPC',
    'Gold'   : 'GLD',
    'USBond' : 'IEF',
}
print("\nDownloading SP500, Gold, IEF from Yahoo Finance (2005-2024)...")
...
for col in yf_data.columns:
    ...
    print(f"  {col:8s}  rows={len(valid)}  "
          f"{valid.index[0].date()} -> {valid.index[-1].date()}")

# STEP 5: Combine all into final daily dataset
...
print("\n" + "="*65)
print("  DATASET OVERVIEW  (IEF - 7-10 Year Treasury)")
print("="*65)
...
print("\n" + "="*65)
print("  STITCH POINT  (Dec 2019 -> Jan 2020)")
print("="*65)
print(data_daily.loc['2019-12-25':'2020-01-10'].to_string())
print("\n" + "="*65)
...
def log_linear_regression(series, name):
    ...
    return {
        'name': name, 'series': s, 'time_yrs': time_yrs,
        'log_price': log_price, 'log_pred': log_pred,
        'price_pred': price_pred, 'residuals': residuals,
        'a': a, 'b': b,
        'CAGR': np.exp(b) - 1,
        ...
        'end_fitted': price_pred[-1], 'years': time_yrs[-1][0],
    }
...
print("="*80)
hdr = f"  {'Asset':<14} {'CAGR':>8} {'Slope b':>12} {'Intercept a':>13} {'Std Dev':>10} {'RMSE':>10} {'R2':>8}"
print(hdr)
print("  " + "-"*78)
...
for name, r in results.items():
    label = 'IEF (7-10Y Treasury)' if name == 'USBond' else name
    print(f"\n{'='*58}")
    if name == 'USBond':
        print(f"  {name}  -- {label}")
    else:
...
for ax, (name, r) in zip(axes, results.items()):
    ...
    ax.plot(r['series'].index, r['log_pred'], color='red', linewidth=2, label=f"OLS fit (CAGR={r['CAGR']*100:.2f}%)")
    ax.set_title(f"{name} - log(Price) vs Time", fontweight='bold')
    ax.set_xlabel("Date")
    ax.set_ylabel("log(Price)")
    ax.legend(fontsize=8)
plt.suptitle("log(Price) vs Time - OLS Regression (2005-2024)\nUSBond = IEF (7-10 Year Treasury ETF)", fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig('plot1_log_full_2005_2024.png', dpi=150, bbox_inches='tight')
print("\nPlot 1 saved: plot1_log_full_2005_2024.png")
...
plt.tight_layout()
plt.savefig('plot2_fitted_full_2005_2024.png', dpi=150, bbox_inches='tight')
print("Plot 2 saved: plot2_fitted_full_2005_2024.png")
...
plt.suptitle("Regression Residuals - log(Price) Space (2005-2024)\nUSBond = IEF (7-10 Year Treasury ETF)", fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig('plot3_resid_full_2005_2024.png', dpi=150, bbox_inches='tight')
print("Plot 3 saved: plot3_resid_full_2005_2024.png")
...
plt.tight_layout()
plt.savefig('plot4_cagr_full_2005_2024.png', dpi=150, bbox_inches='tight')
print("Plot 4 saved: plot4_cagr_full_2005_2024.png")
...
plt.tight_layout()
plt.savefig('plot5_norm_full_2005_2024.png', dpi=150, bbox_inches='tight')
print("Plot 5 saved: plot5_norm_full_2005_2024.png")
