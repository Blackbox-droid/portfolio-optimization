...
print("="*75)
print(f"  {'Asset':<14} {'Intercept a':>13} {'Slope b':>10} "
      f"{'CAGR':>8} {'R²':>8} {'Std Dev':>10} {'Variance':>10}")
print("  " + "-"*73)
...
# ════════════════════════════════════════════════════════════════════
# SECTION 3 — COVARIANCE & CORRELATION  (log returns)
# ════════════════════════════════════════════════════════════════════

# Use daily log returns for covariance / correlation
...
for ax, name in zip(axes, ASSETS):
    ...
    ax.plot(r['series'].index, r['log_pred'],
            ...
            label=f"Fit: a={r['a']:.3f}, b={r['b']:.4f}\n"
                  f"CAGR={r['CAGR']*100:.2f}%  R²={r['R2']:.4f}")
...
ax.set_title("Covariance Matrix — Daily Log Returns  (2005–2024)",
             fontweight='bold')
plt.tight_layout()
plt.show()